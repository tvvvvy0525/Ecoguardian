...
class Genome:
    def __init__(self, penalty=None, radius=None):
        # 基因 1: 拥挤惩罚权重 (0 ~ 1000)
        self.penalty = penalty if penalty is not None else random.uniform(50, 800)
        # 基因 2: 拥挤半径 (1 ~ 8)
    ...
    def mutate(self, rate=0.1):
        ...
        if random.random() < rate:
            self.penalty += random.uniform(-100, 100)
            self.penalty = max(0, min(1000, self.penalty))
        if random.random() < rate:
            self.radius += random.choice([-1, 1])
            self.radius = max(1, min(8, self.radius))
...
class GeneticOptimizer:
    def __init__(self, pop_size=4):
        ...
        # 确保种群里有一个较激进的初始值作为种子
        self.population[0] = Genome(penalty=500, radius=5)
    ...
    def evaluate_fitness(self, genome):
        """计算适应度: 灭火奖励 - 拥挤惩罚"""
        # 逻辑: 我们想要多灭火，且少扎堆
        score = (genome.extinguished_count * 20.0) - (genome.crowded_frames * 1.0)
    ...
    def evolve(self):
        ...
        print(f"Best Genome: Penalty={best.penalty:.1f}, Radius={best.radius}, Score={best.fitness:.1f}")
